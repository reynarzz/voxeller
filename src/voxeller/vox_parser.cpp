...
void vox_parser::parse_nTRN(std::shared_ptr<vox_file> vox, std::ifstream &voxFile)
{
	...
	{
		int keySize = 0;
		voxFile.read(reinterpret_cast<char *>(&keySize), 4);

		std::string key(keySize, '\0');
		voxFile.read(&key[0], keySize);

		int valueSize = 0;
		voxFile.read(reinterpret_cast<char *>(&valueSize), 4);

		std::string value(valueSize, '\0');
		voxFile.read(&value[0], valueSize);

		transform.attributes[std::move(key)] = std::move(value);
	}
	...
	for (size_t i = 0; i < transform.framesCount; i++)
	{
		...
		for (size_t j = 0; j < kvPairFrames; j++)
		{
			...
			if (st.compare("_r") == 0)
			{
				...
				rotMatrix.m00 = row0.x;
				rotMatrix.m01 = row1.x;
				rotMatrix.m02 = row2.x;
				rotMatrix.m10 = row0.y;
				rotMatrix.m11 = row1.y;
				rotMatrix.m12 = row2.y;
				rotMatrix.m20 = row0.z;
				rotMatrix.m21 = row1.z;
				rotMatrix.m22 = row2.z;
